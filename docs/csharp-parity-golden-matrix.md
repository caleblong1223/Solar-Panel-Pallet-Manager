...
| PAR-CORE-005 | History | Filter-combo semantics (`query` + `exact` + date preset + sort mode) produce same result set/order as JS behavior. | `CoreFlowsParityTests.History_FilterComboAndSort_MatchesExpectedRows` |
| PAR-CORE-006 | History | Delete and merge failure paths preserve operator-safe messaging instead of throwing/crashing. | `CoreFlowsParityTests.History_DeleteAndMergeFailures_SetOperatorSafeMessages` |
