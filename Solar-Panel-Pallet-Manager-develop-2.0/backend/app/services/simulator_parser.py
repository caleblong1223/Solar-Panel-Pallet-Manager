...
def _build_result(headers: list[str], rows: list[list[object]]) -> SimulatorParseResult:
    ...
    pm_col = _find_column(headers, aliases={"pm", "pmw", "pm(w)", "pm(watts)"})
    isc_col = _find_column(headers, aliases={"isc", "isc(a)", "isc(amps)"})
    voc_col = _find_column(headers, aliases={"voc", "voc(v)", "voc(volts)"})
    ipm_col = _find_column(headers, aliases={"ipm", "ipm(a)", "ipm(amps)"})
    vpm_col = _find_column(headers, aliases={"vpm", "vpm(v)", "vpm(volts)"})
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        pm = _to_float(row[pm_col]) if pm_col is not None and pm_col < len(row) else None
        isc = _to_float(row[isc_col]) if isc_col is not None and isc_col < len(row) else None
        voc = _to_float(row[voc_col]) if voc_col is not None and voc_col < len(row) else None
        ipm = _to_float(row[ipm_col]) if ipm_col is not None and ipm_col < len(row) else None
        vpm = _to_float(row[vpm_col]) if vpm_col is not None and vpm_col < len(row) else None
        accepted.append(
            ParsedSimRow(
                row_number=idx,
                serial=serial,
