...
export default function LiveBuilderPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // best-effort; builder still works without customers
      }
    })();
  }, [token]);
  ...
  const handleCreatePallet = async (event: FormEvent) => {
    ...
    setIsBusy(true);
    try {
      const created = await createPallet(token, {
        max_panels: parsed,
        template_type: newTemplate,
        customer_id: newCustomerId ? Number(newCustomerId) : undefined,
      });
          ...
          {current ? (
            <div className="builder-meta">
              ...
              <p>
                ...
                {current.customer_id
                  ? customers.find((c) => c.id === current.customer_id)?.display_name ?? `#${current.customer_id}`
                  : "-"}
              </p>
              <p>
