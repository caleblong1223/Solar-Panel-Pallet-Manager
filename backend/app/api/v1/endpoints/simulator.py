...
def _handle_import_request(
    ...
    try:
        _process_simulator_file(db, batch, filename, content)
        batch.status = "completed"
        db.commit()
        db.refresh(batch)
        return SimImportBatchResponse.model_validate(batch)
    except StorageError as exc:
        _fail_batch(db, batch, str(exc))
...
@router.post("/imports/anonymous", response_model=SimImportBatchResponse, status_code=status.HTTP_201_CREATED)
def import_simulator_data_anonymous(
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
) -> SimImportBatchResponse:
    return _handle_import_request(file, db)
